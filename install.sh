...
EOF
    ...
    # Create a symbolic link to ensure NixOS uses the flake
    log "Setting up system to use flake configuration..."
    sudo rm -f /etc/nixos/configuration.nix
    
    # Create a minimal configuration.nix that imports the flake
    sudo tee /etc/nixos/configuration.nix > /dev/null << 'EOF'
# This file imports the flake-based configuration
# Generated by Snowflake installer
{
  imports = [ ./flake.nix ];
}
EOF
...
rebuild_nixos() {
    ...
    # First try with our flake directly
    if ! sudo nixos-rebuild switch --flake ".#$hostname" 2>&1; then
        warn "Direct flake rebuild failed, trying with /etc/nixos flake..."
        # Try with the /etc/nixos flake as fallback
