...
setup_flake_integration() {
    ...
    # Remove any existing flake files to start fresh
    ...
    success "Flake integration configured - configuration copied to /etc/nixos"
}
...
rebuild_nixos() {
    ...
    if ! sudo nixos-rebuild switch --flake ".#$hostname" 2>&1; then
        warn "Flake rebuild failed, trying alternative method..."
